...
@section('breadcrumb')
    <nav aria-label="breadcrumb">
        ...
        <h5 class="font-weight-bolder mb-0">Gestión de Roles</h5>
    </nav>
@stop
@section('content')
    @can('Role access')
                    ...
                    <div class="row">
                        <div class="col-lg-6 col-7">
                            ...
                            <p class="text-sm mb-0">
                                ...
                                <span class="font-weight-bold ms-1">{{ $roles->count() }}</span> disponibles
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div id="table" class="table-responsive p-0 navbar-nav-scroll">
                        <table class="table scroller position-relative max-height-vh-100 h-100 align-items-center mb-0">
                            ...
                            <tr>
                                ...
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-1">Permisos</th>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center h-auto">
                                        ...
                                        <form id="roleDelete" action="{{ route('admin.roles.destroy', $role->id) }}" method="POST" class="inline">
                                            ...
                                            @method('DELETE')
                                            @can('Role edit')
                                            <a href="{{ route('admin.roles.edit', $role->id) }}" class="mx-3" data-bs-toggle="tooltip" data-bs-original-title="Editar rol">
                                                <i class="fa-solid fa-wand-magic-sparkles text-secondary"></i>
                                            </a>
                                            @endcan
                                            @can('Role delete')
                                            <button class="cursor-pointer fas fa-trash text-secondary" style="border: none; background: no-repeat;" data-bs-toggle="tooltip" data-bs-original-title="Eliminar rol"></button>
                                            @endcan
                                        </form>
                                        @endif
                                    </td>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header pb-0 px-3">
                    <h5 class="mb-0">{{ __('Agregar Nuevo Rol') }}</h5>
                </div>
                <div class="card-body pt-4 p-3">
                    <form method="POST" action="{{ route('admin.roles.store') }}" role="form text-left">
                        ...
                        <div class="form-group">
                            <label for="name" class="form-control-label">{{ __('Nombre del Rol') }}</label>
                            <div class="@error('name')border border-danger rounded-3 @enderror">
                                <input class="form-control" type="text" placeholder="Nombre del rol" id="name" name="name" required>
                                @error('name')
                        ...
                        <div class="form-group">
                            <label for="name" class="form-control-label">{{ __('Asignar Permisos') }}</label>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn bg-gradient-info btn-md mt-4 mb-4">{{ 'Agregar Rol' }}</button>
                        </div>
                        </div>
                    </form>
                </div>
